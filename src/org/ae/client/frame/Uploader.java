...
public class Uploader extends Composite implements IsUpdateable, IUploader,
  ...
  public static final UploaderConstants I18N_CONSTANTS = GWT
      .create(UploaderConstants.class);
   ...
   * this period is reached, the upload process is cancelled.
   */
  ...
  private final Timer automaticUploadTimer = new Timer() {
    ...
    public void run() {
      if (isTheFirstInQueue()) {
        this.cancel();
        firstTime = true;
        getMessageBox().updateText(IUploadStatus.Status.SUBMITING.toString());
    ...
    public void onResponseReceived(Request request, Response response) {
      ...
      try {
        ...
        if (text.contains("<blobpath>")) {
          url = text.replaceAll("[\r\n]+", "")
              .replaceAll("^.*<blobpath>\\s*", "")
              .replaceAll("\\s*</blobpath>.*$", "");
        }
      } catch (Exception e) {
  ...
  private final RequestCallback onCancelReceivedCallback = new RequestCallback() {
    public void onError(Request request, Throwable exception) {
      ...
      getMessageBox().updateText(IUploadStatus.Status.CANCELED.toString());
  ...
  private final RequestCallback onDeleteFileCallback = new RequestCallback() {
    public void onError(Request request, Throwable exception) {
      getMessageBox().updateText(Status.DELETED.toString());
  ...
  private final ChangeHandler onFileInputChanged = new ChangeHandler() {
    public void onChange(ChangeEvent event) {
      ...
      if (avoidRepeatedFiles && fileDone.contains(getFileName())) {
        getMessageBox().updateText(Status.REPEATED.toString());
  ...
  private final RequestCallback onSessionReceivedCallback = new RequestCallback() {
    ...
    public void onResponseReceived(Request request, Response response) {
      hasSession = true;
      try {
        String s = Utils.getXmlNodeValue(XMLParser.parse(response.getText()),
            "blobstore");
  ...
  private final RequestCallback onStatusReceivedCallback = new RequestCallback() {
    public void onError(Request request, Throwable exception) {
      ...
      if (exception instanceof RequestTimeoutException) {
        log("GWTUpload: onStatusReceivedCallback timeout error, asking the server again.",
            null);
      } else {
        log("GWTUpload: onStatusReceivedCallback error: "
        ...
        message += "\n" + exception.toString();
        getMessageBox().close();
        statusWidget.setError(i18nStrs.uploaderServerUnavailable()
            + getServletPath() + "\n\n" + message);
      }
    }
  ...
  private SubmitCompleteHandler onSubmitCompleteHandler = new SubmitCompleteHandler() {
    public void onSubmitComplete(SubmitCompleteEvent event) {
      ...
      if (serverResponse != null) {
        serverResponse = serverResponse.replaceFirst(
            ".*%%%INI%%%([\\s\\S]*?)%%%END%%%.*", "$1");
        serverResponse = serverResponse.replace("@@@", "<").replace("___", ">")
            .replace("&lt;", "<").replaceAll("&gt;", ">");
      }
      ...
      try {
        ...
        // Otherwise force an ajax request so as we have not to wait to the
        // timer schedule
   ...
   * 
   * If the client hasn't got the session, it asks for a new one and the submit
   * process is delayed until the client has got it
   */
  private SubmitHandler onSubmitFormHandler = new SubmitHandler() {
    public void onSubmit(SubmitEvent event) {
      ...
      if (!finished && uploading) {
        uploading = false;
        getMessageBox().updateText(Status.CANCELED.toString());
        statusWidget.setStatus(IUploadStatus.Status.CANCELED);
        return;
      }
      ...
      getMessageBox().updateText(Status.INPROGRESS.toString());
  ...
  private final UpdateTimer updateStatusTimer = new UpdateTimer(this,
      statusInterval);
  ...
  /**
   * Default constructor. Initialize widget components and layout elements using
  ...
  /**
   * Initialize widget components and layout elements using the standard file
   * input.
   * 
   ...
   * 
   * @see
   * gwtupload.client.IUploader#addOnCancelUploadHandler(gwtupload.client.IUploader
   * .OnCancelUploaderHandler)
   */
  public HandlerRegistration addOnCancelUploadHandler(
      final OnCancelUploaderHandler handler) {
    assert handler != null;
    return statusWidget
        .addCancelHandler(new IUploadStatus.UploadCancelHandler() {
          public void onCancel() {
   ...
   * 
   * @see
   * gwtupload.client.IUploader#addOnChangeUploadHandler(gwtupload.client.IUploader
   * .OnChangeUploaderHandler)
   */
  public HandlerRegistration addOnChangeUploadHandler(
      final IUploader.OnChangeUploaderHandler handler) {
    assert handler != null;
   ...
   * 
   * @see
   * gwtupload.client.IUploader#addOnFinishUploadHandler(gwtupload.client.IUploader
   * .OnFinishUploaderHandler)
   */
  public HandlerRegistration addOnFinishUploadHandler(
      final IUploader.OnFinishUploaderHandler handler) {
    assert handler != null;
   ...
   * 
   * @see
   * gwtupload.client.IUploader#addOnStartUploadHandler(gwtupload.client.IUploader
   * .OnStartUploaderHandler)
   */
  public HandlerRegistration addOnStartUploadHandler(
      final IUploader.OnStartUploaderHandler handler) {
    assert handler != null;
   ...
   * 
   * @see
   * gwtupload.client.IUploader#addOnStatusChangedHandler(gwtupload.client.IUploader
   * .OnStatusChangedHandler)
   */
  public HandlerRegistration addOnStatusChangedHandler(
      final OnStatusChangedHandler handler) {
    assert handler != null;
  ...
  public void cancel() {
    ...
    if (uploading) {
      ...
      getMessageBox().updateText(Status.CANCELING.toString());
  ...
  /**
   * Returns the link for getting the uploaded file from the server It's useful
   * to display uploaded images or generate links to uploaded files.
   */
   ...
   * It's useful in the exported version of the library. Because native
   * javascript needs it
   */
  public JavaScriptObject getData() {
    return getDataImpl(fileUrl(), getInputName(), getFileName(), getBasename(),
        getServerResponse(), getServerInfo().message, getStatus().toString());
  }
   ...
   * 
   * @see
   * gwtupload.client.IUploader#setI18Constants(gwtupload.client.I18nUploadConstants
   * )
   */
  ...
  /**
   * set the url of the server service that receives the files and informs about
   * the progress.
   */
  ...
  public void update() {
    try {
      ...
      // So it's better to change the request path sending an additional random
      // parameter
      RequestBuilder reqBuilder = new RequestBuilder(RequestBuilder.GET,
          composeURL("filename=" + fileInput.getName(), "c="
              + requestsCounter++));
  ...
  /**
   * Method called when the file input has changed. This happens when the user
   * selects a file.
   * 
   * Override this method if you want to add a customized behavior, but remember
   * to call this in your function
   */
  protected void onChangeInput() {
    getMessageBox().updateText(Status.CHANGED.toString());
  ...
  /**
   * Method called when the file upload process has finished, or the file has
   * been canceled or removed from the queue. Override this method if you want
   * to add a customized behavior, but remember to call this in your function.
   */
  ...
  /**
   * Method called when the file is added to the upload queue. Override this if
   * you want to add a customized behavior, but remember to call this from your
   * method.
   */
  ...
  private void addToQueue() {
    getMessageBox().updateText(Status.QUEUED.toString());
  ...
  /**
   * Change the fileInput name, because the server uses it as an uniq
   * identifier.
   */
  ...
  private native JavaScriptObject getDataImpl(String url, String inputName,
      String fileName, String baseName, String serverResponse,
      String serverMessage, String status) /*-{
		return {
			url : url,
			name : inputName,
			filename : fileName,
  ...
  private void parseAjaxResponse(String responseTxt) {
    ...
    if (error != null) {
      ...
      if (serverResponse != null) {
        log("server response received, cancelling the upload " + getFileName()
            + " " + serverResponse, null);
        successful = true;
        uploadFinished();
      }
      ...
      lastData = now();
      int transferredKB = Integer.valueOf(Utils.getXmlNodeValue(doc,
          TAG_CURRENT_BYTES)) / 1024;
      int totalKB = Integer
          .valueOf(Utils.getXmlNodeValue(doc, TAG_TOTAL_BYTES)) / 1024;
      statusWidget.setProgress(transferredKB, totalKB);
      log("server response transferred  " + transferredKB + "/" + totalKB + " "
          + getFileName(), null);
      return;
    } else {
  ...
  private void sendAjaxRequestToDeleteUploadedFile() throws RequestException {
    RequestBuilder reqBuilder = new RequestBuilder(RequestBuilder.GET,
        composeURL("remove=" + getInputName()));
    reqBuilder.sendRequest("remove_file", onDeleteFileCallback);
  }

  /**
   * Sends a request to the server in order to get the blobstore path. When the
   * response with the session comes, it submits the form.
   */
  private void sendAjaxRequestToGetBlobstorePath() throws RequestException {
    RequestBuilder reqBuilder = new RequestBuilder(RequestBuilder.GET,
        getServletPath());// composeURL("blobstore=true"));
    reqBuilder.setTimeoutMillis(DEFAULT_AJAX_TIMEOUT);
    reqBuilder.sendRequest("blobstore", onBlobstoreReceivedCallback);
  }

  /**
   * Sends a request to the server in order to get the session cookie, when the
   * response with the session comes, it submits the form.
   * 
   ...
   * the client at the end of the request, and in the meanwhile the client needs
   * to know the session in order to ask the server for the upload status.
   */
  private void sendAjaxRequestToValidateSession() throws RequestException {
    // Using a reusable builder makes IE fail
    RequestBuilder reqBuilder = new RequestBuilder(RequestBuilder.GET,
        composeURL("new_session=true"));
    reqBuilder.setTimeoutMillis(DEFAULT_AJAX_TIMEOUT);
    reqBuilder.sendRequest("create_session", onSessionReceivedCallback);
  }

  /**
   * Called when the uploader detects that the upload process has finished: - in
   * the case of submit complete. - in the case of error talking with the
   * server.
   */
  ...
  private boolean validateExtension(String filename) {
    ...
    if (!valid) {
      ...
      statusWidget.setError(i18nStrs.uploaderInvalidExtension()
          + validExtensionsMsg);
    }
    return valid;
  }
  ...
  protected MessageBox getMessageBox() {
    if (messageBox == null) {
      ...
      messageBox = MessageBox.wait("Uploading Image",
          "Uploading, please wait...", "Loading...");
    }
    return messageBox;
  }
